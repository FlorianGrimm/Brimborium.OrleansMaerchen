...
namespace Orleans.DurableTask.Core.Tests;
...
public class ExceptionHandlingIntegrationTests {
    ...
    public async Task CatchInvalidOperationException(ErrorPropagationMode mode) {
        ...
        if (mode == ErrorPropagationMode.SerializeExceptions) {
            ...
            InvalidOperationException? e =
                string.IsNullOrEmpty(state.Output)
                ? default
                : JsonConvert.DeserializeObject<InvalidOperationException>(state.Output);
            Assert.IsNotNull(e);
            ...
            FailureDetails? details = string.IsNullOrEmpty(state.Output)
                ? default
                : JsonConvert.DeserializeObject<FailureDetails>(state.Output);
    ...
    public async Task FailureDetailsOnHandled() {
        ...
        int retryPolicyInvokedCount = string.IsNullOrEmpty(state.Output)
                ? default
                : JsonConvert.DeserializeObject<int>(state.Output);
