...
public class TaskOrchestrationDispatcher {
    private readonly INameVersionObjectManager<TaskOrchestration> _ObjectManager;
    ...
    private ErrorPropagationMode _ErrorPropagationMode;
    private readonly NonBlockingCountdownLock _ConcurrentSessionLock;
    ...
    private readonly TaskOrchestrationEntityParameters? _EntityParameters;
    ...
    internal TaskOrchestrationDispatcher(
        ...
        this._EntityParameters = TaskOrchestrationEntityParameters.FromEntityBackendProperties(this._EntityBackendProperties);
        ...
        this._ConcurrentSessionLock = new NonBlockingCountdownLock(maxConcurrentSessions);
    }
    ...
    protected async Task<TaskOrchestrationWorkItem?> OnFetchWorkItemAsync(TimeSpan receiveTimeout, CancellationToken cancellationToken) {
        if (this._EntityBackendProperties?.UseSeparateQueueForEntityWorkItems == true) {
            ...
            return await this._EntityOrchestrationService!.LockNextOrchestrationWorkItemAsync(receiveTimeout, cancellationToken);
        } else {
            ...
            return await this._OrchestrationService.LockNextTaskOrchestrationWorkItemAsync(receiveTimeout, cancellationToken);
        }
    }
...
#warning TODO:!
    ...
    private async Task OnProcessWorkItemSessionAsync(TaskOrchestrationWorkItem workItem) {
        ...
        this.EnsureExecutionStartedIsFirst(workItem.NewMessages);
    ...
    protected async Task<bool> OnProcessWorkItemAsync(TaskOrchestrationWorkItem workItem) {
        ...
        workItem.OrchestrationRuntimeState.LogHelper = this._LogHelper;
        var runtimeState = workItem.OrchestrationRuntimeState;

        runtimeState.AddEvent(new OrchestratorStartedEvent(-1));

        var originalOrchestrationRuntimeState = runtimeState;
        ...
        if (workItem.LockedUntilUtc < DateTime.MaxValue) {
            ...
            renewTask = Task.Factory.StartNew(
                () => RenewUntil(workItem, this._OrchestrationService, this._LogHelper, nameof(TaskOrchestrationDispatcher), renewCancellationTokenSource.Token),
        ...
        try {
            ...
            if (!ReconcileMessagesWithState(workItem, nameof(TaskOrchestrationDispatcher), this._ErrorPropagationMode, this._LogHelper)) {
                ...
                do {
                    ...
                    this._LogHelper.OrchestrationExecuting(runtimeState.OrchestrationInstance!, runtimeState.Name);
                    ...
                    this._LogHelper.OrchestrationExecuted(
                        runtimeState.OrchestrationInstance!,
                        runtimeState.Name,
                        decisions);
                    TraceHelper.TraceInstance(
                    ...
                    foreach (OrchestratorAction decision in decisions) {
                        ...
                        if (this._OrchestrationService.IsMaxMessageCountExceeded(totalMessages, runtimeState)) {
                            TraceHelper.TraceInstance(
                                TraceEventType.Information,
...
#warning TODO:!
    ...
    private static TimeSpan MinRenewalInterval = TimeSpan.FromSeconds(5); // prevents excessive retries if clocks are off
    private static TimeSpan MaxRenewalInterval = TimeSpan.FromSeconds(30);
    ...
    internal static async Task RenewUntil(TaskOrchestrationWorkItem workItem, IOrchestrationService orchestrationService, LogHelper logHelper, string dispatcher, CancellationToken cancellationToken) {
        while (!cancellationToken.IsCancellationRequested) {
            ...
            try {
                ...
                _ = TraceHelper.TraceException(TraceEventType.Warning, $"{dispatcher}-RenewWorkItemFailed", exception, "Failed to renew work item for instance {0}", workItem.InstanceId);
            }
        }
    }
    ...
    private async Task<OrchestrationExecutionCursor> ExecuteOrchestrationAsync(OrchestrationRuntimeState runtimeState, TaskOrchestrationWorkItem workItem) {
        ...
        TaskOrchestration? taskOrchestration = this._ObjectManager.GetObject(runtimeState.Name, runtimeState.Version!);
        ...
        dispatchContext.SetProperty(this._EntityParameters);

        TaskOrchestrationExecutor? executor = null;

        await this._DispatchPipeline.RunAsync(dispatchContext, _ => {
            ...
            executor = new TaskOrchestrationExecutor(
                runtimeState,
                taskOrchestration,
                this._OrchestrationService.EventBehaviourForContinueAsNew,
                this._EntityParameters,
                this._ErrorPropagationMode);
        ...
        return new OrchestrationExecutionCursor(
            runtimeState, 
#warning TODO:!
            taskOrchestration!,
#warning TODO:!
    ...
    private async Task ResumeOrchestrationAsync(TaskOrchestrationWorkItem workItem) {
        ArgumentNullException.ThrowIfNull(workItem.Cursor, nameof(workItem));
            ...
            foreach (TaskMessage message in workItem.NewMessages) {
                ...
                if (Activity.Current != null) {
                    ...
                    if (historyEvent is TimerFiredEvent timerFiredEvent) {
                        ...
                        var subOrchestrationCreatedEvent = (SubOrchestrationInstanceCreatedEvent?)workItem.OrchestrationRuntimeState.Events.FirstOrDefault(x => x.EventId == subOrchestrationInstanceCompletedEvent.TaskScheduledId);
...
#warning TODO:!
        ...
        this._LogHelper.OrchestrationCompleted(runtimeState, completeOrchestratorAction);
        TraceHelper.TraceInstance(
...
#warning TODO:!
                ...
                if (completeOrchestratorAction.OrchestrationStatus == OrchestrationStatus.Failed) {
                    _ = (DistributedTraceActivity.Current?.SetStatus(
                        ActivityStatusCode.Error, completeOrchestratorAction.Result));
                }
            }

            this.ResetDistributedTraceActivity(runtimeState);
...
#warning TODO:!
        ...
        this._LogHelper.SchedulingActivity(
            runtimeState.OrchestrationInstance!,
            scheduledEvent);
...
#warning TODO:!
        ...
        this._LogHelper.SchedulingOrchestration(startedEvent);
...
#warning TODO:!
        ...
        this._LogHelper.RaisingEvent(runtimeState.OrchestrationInstance!, historyEvent);
...
#warning TODO:!
    ...
    internal class NonBlockingCountdownLock {
        ...
        public bool Acquire() {
            ...
            _ = Interlocked.Increment(ref this.available);
            return false;
        }
