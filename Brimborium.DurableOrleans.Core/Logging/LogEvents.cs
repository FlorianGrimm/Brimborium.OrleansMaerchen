...
namespace Orleans.DurableTask.Core.Logging;
...
internal static class LogEvents {
    ...
    internal class DispatchersStopping : StructuredLogEvent, IEventSourceEvent {
        ...
        protected override string CreateLogMessage() =>
            $"{this.Dispatcher}: Dispatchers are draining. Remaining work items: {this.WorkItemCount}. Remaining work item fetchers: {this.ActiveFetcherCount}";
    ...
    internal class FetchWorkItemStarting : StructuredLogEvent, IEventSourceEvent {
        ...
        protected override string CreateLogMessage() =>
            $"{this.Dispatcher}: Fetching next work item. Current active work-item count: {this.WorkItemCount}. Maximum active work-item count: {this.MaxWorkItemCount}. Timeout: {this.TimeoutSeconds}s";
    ...
    internal class FetchWorkItemCompleted : StructuredLogEvent, IEventSourceEvent {
        ...
        protected override string CreateLogMessage() =>
            $"{this.Dispatcher}: Fetched next work item '{this.WorkItemId}' after {this.LatencyMs}ms. Current active work-item count: {this.WorkItemCount}. Maximum active work-item count: {this.MaxWorkItemCount}";
    ...
    internal class FetchingThrottled : StructuredLogEvent, IEventSourceEvent {
        ...
        protected override string CreateLogMessage() =>
            $"{this.Dispatcher}: Delaying work item fetching because the current active work-item count ({this.WorkItemCount}) exceeds the configured maximum active work-item count ({this.MaxWorkItemCount})";
    ...
    internal class RaisingEvent : StructuredLogEvent, IEventSourceEvent {
        public RaisingEvent(OrchestrationInstance target, EventRaisedEvent raisedEvent) {
            this.Name = raisedEvent.Name ?? string.Empty;
        ...
        public RaisingEvent(OrchestrationInstance source, EventSentEvent sentEvent) {
            ...
            this.ExecutionId = source.ExecutionId ?? string.Empty;
            this.TargetInstanceId = sentEvent.InstanceId ?? string.Empty;
    ...
    internal class SuspendingInstance : StructuredLogEvent, IEventSourceEvent {
        public SuspendingInstance(OrchestrationInstance instance, string? reason) {
            this.InstanceId = instance.InstanceId;
            this.ExecutionId = instance.ExecutionId ?? string.Empty;
            this.Details = reason ?? string.Empty;
        }
...
#warning TODO:!
    ...
    internal class RenewActivityMessageFailed : StructuredLogEvent, IEventSourceEvent {
        public RenewActivityMessageFailed(TaskActivityWorkItem? workItem, Exception exception) {
            this.InstanceId = workItem?.TaskMessage.OrchestrationInstance.InstanceId ?? string.Empty;
            this.ExecutionId = workItem?.TaskMessage.OrchestrationInstance.ExecutionId ?? string.Empty;

            var taskEvent = workItem?.TaskMessage.Event as TaskScheduledEvent;
            this.Name = taskEvent?.Name ?? string.Empty;
            this.TaskEventId = taskEvent?.EventId ?? 0;
            this.Details = exception.ToString();
        }
