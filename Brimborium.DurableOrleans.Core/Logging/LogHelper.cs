...
internal class LogHelper {
    ...
    private bool IsStructuredLoggingEnabled => this.log != null;
    ...
    internal static string GetRedactedExceptionDetails(Exception? exception) {
        ...
        _ = sb.Append(exception.GetType().FullName).Append(": ").Append("[Redacted]");
        if (exception.InnerException != null) {
            // Recursive
            _ = sb.AppendLine().Append(" ---> ").AppendLine(GetRedactedExceptionDetails(exception.InnerException));
            _ = sb.Append("   --- End of inner exception stack trace ---");
        }

        _ = sb.AppendLine().Append(exception.StackTrace);
        return sb.ToString();
    }
}
