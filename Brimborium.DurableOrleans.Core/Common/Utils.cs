...
namespace Orleans.DurableTask.Core.Common;
...
#warning TODO: StringBuilder Pooling
...
#warning TODO: Memory Pooling
        ...
        */
        ...
        memoryStream.Position = 0;
        originalStreamSize = memoryStream.Length;

        if (compress) {
            var compressedStream = GetCompressedStream(memoryStream)!;
    ...
    public static byte[] ReadBytesFromStream(Stream objectStream) {
        ...
        _ = objectStream.Read(serializedBytes, 0, serializedBytes.Length);
        objectStream.Position = 0;
...
#warning TODO: pooling
    ...
    public static async Task ExecuteWithRetries(Func<Task> retryAction, string sessionId, string operation,
        ...
        TraceHelper.Trace(TraceEventType.Error, "ExecuteWithRetry-RetriesExhausted", "Exhausted all retries for operation {0}", operation);
    ...
    public static async Task<T?> ExecuteWithRetries<T>(Func<Task<T>> retryAction, string sessionId, string operation,
        ...
        TraceHelper.Trace(TraceEventType.Error, eventType, "Exhausted all retries for operation {0}", operation);
    ...
    public static string? SerializeCause(Exception originalException, DataConverter converter) {
        ArgumentNullException.ThrowIfNull(originalException, nameof(originalException));
    ...
    public static Exception? RetrieveCause(string? details, DataConverter converter) {
        ...
        try {
            if (!string.IsNullOrWhiteSpace(details)) {
                return converter.Deserialize<Exception>(details).GetValueOrNull();
            } else {
                return null;
            }
        } catch (Exception converterException) when (!IsFatal(converterException)) {
            return new TaskFailedExceptionDeserializationException(details, converterException);
        }
    }
    ...
    public static string EscapeJson(string inputJson) {
        return new StringBuilder(inputJson)
            .Replace("{", "{{")
            .Replace("}", "}}")
            .Replace(";", "%3B")
            .Replace("=", "%3D")
    ...
    public static Task DelayWithCancellation(TimeSpan timeout, CancellationToken cancellationToken) {
        ...
        return Task.WhenAny(Task.Delay(timeout, CancellationToken.None), tcs.Task);
    }
...
#warning TODO: not sure whats wrong implementation or documentation
    ...
    public static bool TryGetTaskScheduledId(HistoryEvent historyEvent, out int taskScheduledId) {
        switch (historyEvent.EventType) {
            ...
            case EventType.SubOrchestrationInstanceCompleted:
                ...
                taskScheduledId = subOrchestrationInstanceCompletedEvent.TaskScheduledId;
                return true;
            case EventType.SubOrchestrationInstanceFailed:
                if (historyEvent is not SubOrchestrationInstanceFailedEvent subOrchestrationInstanceFailedEvent) {
                    throw new ArgumentException(historyEvent.EventType.ToString(), nameof(historyEvent));
                }
                taskScheduledId = subOrchestrationInstanceFailedEvent.TaskScheduledId;
                return true;
            case EventType.TimerFired:
                if (historyEvent is not TimerFiredEvent timerFiredEvent) {
                    throw new ArgumentException(historyEvent.EventType.ToString(), nameof(historyEvent));
                }
                taskScheduledId = timerFiredEvent.TimerId;
                return true;
            default:
                taskScheduledId = -1;
                return false;
...
#endif
            if (typeToConvert.GetElementType() is Type elementType) {
