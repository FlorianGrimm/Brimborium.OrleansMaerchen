...
public class OrchestrationRuntimeState {
    ...
    private readonly HashSet<int> _CompletedEventIds;
    ...
    public OrchestrationRuntimeState(IList<HistoryEvent>? events) {
        this.Events = events is not null ? new List<HistoryEvent>(events.Count) : new List<HistoryEvent>();
        this.PastEvents = events is not null ? new List<HistoryEvent>(events.Count) : new List<HistoryEvent>();
        this.NewEvents = new List<HistoryEvent>();
        this._CompletedEventIds = new HashSet<int>();
        this._OrchestrationStatus = OrchestrationStatus.Running;
    ...
    /// </summary>
    public DateTime CreatedTime => this.GetExecutionStartedEventOrThrow().Timestamp;
    ...
    /// </summary>
    public string? Input => this.GetExecutionStartedEventOrThrow().Input;

    /// <summary>
    /// Gets the serialized output of the ExecutionCompletedEvent if completed else null
    /// </summary>
    public string? Output => this.ExecutionCompletedEvent?.FailureDetails?.ToString() ?? this.ExecutionCompletedEvent?.Result;
    ...
    /// </remarks>
    public FailureDetails? FailureDetails => this.ExecutionCompletedEvent?.FailureDetails;

    /// <summary>
    /// Gets the orchestration name from the ExecutionStartedEvent
    /// </summary>
    public string Name => this.GetExecutionStartedEventOrThrow().Name;

    /// <summary>
    /// Gets the orchestration version of the ExecutionStartedEvent
    /// </summary>
    public string Version => this.GetExecutionStartedEventOrThrow().Version;
    ...
    public OrchestrationStatus OrchestrationStatus {
        get {
            _ = this.GetExecutionStartedEventOrThrow();
    ...
    public bool IsValid =>
        ...
        || (this.ExecutionStartedEvent is not null);
    ...
    /// </remarks>
    internal LogHelper? LogHelper { get; set; }
    ...
    private ExecutionStartedEvent GetExecutionStartedEventOrThrow() {
        var executionStartedEvent = this.ExecutionStartedEvent;
    ...
    private bool IsDuplicateEvent(HistoryEvent historyEvent) {
        if (historyEvent.EventId >= 0 &&
            ...
            !this._CompletedEventIds.Add(historyEvent.EventId)) {
    ...
    private void SetMarkerEvents(HistoryEvent historyEvent) {
        if (historyEvent is ExecutionStartedEvent startedEvent) {
            if (this.ExecutionStartedEvent is not null) {
            ...
            if (this.ExecutionCompletedEvent is null) {
                this.ExecutionCompletedEvent = completedEvent;
                this._OrchestrationStatus = completedEvent.OrchestrationStatus;
            } else {
                ...
                var log = $"Received new {completedEvent.GetType().Name} event despite the orchestration being already in the {this._OrchestrationStatus} state.";

                if (this._OrchestrationStatus == OrchestrationStatus.ContinuedAsNew && completedEvent.OrchestrationStatus == OrchestrationStatus.Terminated) {
                ...
                this.LogHelper?.OrchestrationDebugTrace(this.OrchestrationInstance?.InstanceId ?? "", this.OrchestrationInstance?.ExecutionId ?? "", log);
            }
        } else if (historyEvent is ExecutionSuspendedEvent) {
            this._OrchestrationStatus = OrchestrationStatus.Suspended;
