...
public abstract class TraceContextBase {
    ...
    /// </summary>
    [JsonIgnore]
    public /* internal */ Activity CurrentActivity { get; set; }
    ...
    public static TraceContextBase Restore(string json) {
        ...
        var typeNameStr = typeName.Value<string>();
        traceContextType = Type.GetType(typeNameStr);
        
        if (traceContextType is null
            || !traceContextType.IsSubclassOf(traceContextBasetype)) {
