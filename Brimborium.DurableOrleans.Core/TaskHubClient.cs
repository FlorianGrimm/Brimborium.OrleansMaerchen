...
public sealed class TaskHubClient {
    private readonly DataConverter _DefaultConverter;
    private readonly LogHelper _LogHelper;

    internal LogHelper LogHelper => this._LogHelper;
    internal DataConverter DefaultConverter => this._DefaultConverter;
    ...
    public TaskHubClient(
        ...
        this._DefaultConverter = dataConverter ?? JsonDataConverter.Default;
        this._LogHelper = new LogHelper(loggerFactory?.CreateLogger("DurableTask.Core"));
    }
    ...
    private async Task<OrchestrationInstance> InternalCreateOrchestrationInstanceWithRaisedEventAsync(
        ...
        string? serializedOrchestrationData = this._DefaultConverter.SerializeInternal(orchestrationInput);
        ...
        if (eventData != null) {
            string? serializedEventData = this._DefaultConverter.SerializeInternal(eventData);
            var eventRaisedEvent = new EventRaisedEvent(-1, serializedEventData) {
                Name = eventName
            };

            this._LogHelper.RaisingEvent(orchestrationInstance, eventRaisedEvent);
    ...
    private TraceContextBase CreateOrExtractRequestTraceContext(string? eventName) {
        ...
        {
            var requestTraceContext = TraceContextFactory.Create($"{TraceConstants.Client}: {eventName}");
            requestTraceContext.StartAsNew();
            return requestTraceContext;
        } else {
            var requestTraceContext = TraceContextFactory.Create(Activity.Current);
            return requestTraceContext;
        }
    }
    ...
    public async Task RaiseEventAsync(OrchestrationInstance orchestrationInstance, string eventName, object eventData) {
        ...
        string serializedInput = this._DefaultConverter.SerializeInternal(eventData);

        // Distributed Tracing
        EventRaisedEvent eventRaisedEvent = new EventRaisedEvent(-1, serializedInput) { Name = eventName };
        using var traceActivity = TraceHelper.StartActivityForNewEventRaisedFromClient(eventRaisedEvent, orchestrationInstance);
        ...
        this._LogHelper.RaisingEvent(orchestrationInstance, (EventRaisedEvent)taskMessage.Event);
    ...
    public async Task TerminateInstanceAsync(OrchestrationInstance orchestrationInstance, string reason) {
        ...
        this._LogHelper.TerminatingInstance(orchestrationInstance, reason);
    ...
    public async Task SuspendInstanceAsync(OrchestrationInstance orchestrationInstance, string? reason = null) {
        ...
        this._LogHelper.SuspendingInstance(orchestrationInstance, reason);
    ...
    public async Task ResumeInstanceAsync(OrchestrationInstance orchestrationInstance, string? reason = null) {
        ...
        this._LogHelper.ResumingInstance(orchestrationInstance, reason);
    ...
    public Task<OrchestrationState> WaitForOrchestrationAsync(
        ...
        TimeSpan timeout) {
        //if (string.IsNullOrWhiteSpace(orchestrationInstance?.InstanceId)) {
        //    throw new ArgumentException(null, nameof(orchestrationInstance));
        //}
        ArgumentException.ThrowIfNullOrWhiteSpace(orchestrationInstance?.InstanceId, nameof(orchestrationInstance));

        this._LogHelper.WaitingForInstance(orchestrationInstance, timeout);
    ...
    public async Task<OrchestrationState?> GetOrchestrationStateAsync(string instanceId) {
        var state = await this.GetOrchestrationStateAsync(instanceId, false);
    ...
    public Task<IList<OrchestrationState>> GetOrchestrationStateAsync(string instanceId, bool allExecutions) {
        this._LogHelper.FetchingInstanceState(instanceId);
    ...
    public Task<OrchestrationState?> GetOrchestrationStateAsync(OrchestrationInstance instance) {
        ArgumentException.ThrowIfNullOrEmpty(instance?.ExecutionId, nameof(instance));
    ...
    public Task<OrchestrationState?> GetOrchestrationStateAsync(string instanceId, string executionId) {
        this._LogHelper.FetchingInstanceState(instanceId, executionId);
    ...
    public Task<string> GetOrchestrationHistoryAsync(OrchestrationInstance instance) {
        ...
        this._LogHelper.FetchingInstanceHistory(instance);
