...
public class TraceHelper {
    ...
    internal static Activity? StartActivityForNewOrchestration(ExecutionStartedEvent startEvent) {
        ...
        if (newActivity != null) {
            _ = newActivity.SetTag(Schema.Task.Type, TraceActivityConstants.Orchestration);
            _ = newActivity.SetTag(Schema.Task.Name, startEvent.Name);
            _ = newActivity.SetTag(Schema.Task.InstanceId, startEvent.OrchestrationInstance.InstanceId);
            _ = newActivity.SetTag(Schema.Task.ExecutionId, startEvent.OrchestrationInstance.ExecutionId);

            if (!string.IsNullOrEmpty(startEvent.Version)) {
                _ = newActivity.SetTag(Schema.Task.Version, startEvent.Version);
            }
    ...
    internal static Activity? StartTraceActivityForOrchestrationExecution(ExecutionStartedEvent? startEvent) {
        ...
        if (startEvent.ParentTraceContext is null) {
            throw new InvalidOperationException("startEvent.ParentTraceContext is null");
        } else if (startEvent.ParentTraceContext.Id != null && startEvent.ParentTraceContext.SpanId != null) {
    ...
    internal static Activity? StartTraceActivityForTaskExecution(
        ...
        _ = newActivity.SetTag(Schema.Task.Type, TraceActivityConstants.Activity);
        _ = newActivity.SetTag(Schema.Task.Name, scheduledEvent.Name);
        _ = newActivity.SetTag(Schema.Task.InstanceId, instance.InstanceId);
        _ = newActivity.SetTag(Schema.Task.TaskId, scheduledEvent.EventId);

        if (!string.IsNullOrEmpty(scheduledEvent.Version)) {
            _ = newActivity.SetTag(Schema.Task.Version, scheduledEvent.Version);
        }
    ...
    internal static Activity? StartTraceActivityForSchedulingTask(
        ...
        _ = newActivity.AddTag(Schema.Task.Type, TraceActivityConstants.Activity);
        _ = newActivity.AddTag(Schema.Task.Name, taskScheduledEvent.Name);
        _ = newActivity.AddTag(Schema.Task.InstanceId, instance?.InstanceId);
        _ = newActivity.AddTag(Schema.Task.TaskId, taskScheduledEvent.EventId);

        if (!string.IsNullOrEmpty(taskScheduledEvent.Version)) {
            _ = newActivity.AddTag(Schema.Task.Version, taskScheduledEvent.Version);
        }
    ...
    internal static void EmitTraceActivityForTaskFailed(
        ...
        if (failedEvent != null) {
            ...
            _ = (activity?.SetStatus(ActivityStatusCode.Error, statusDescription));
        }
    ...
    internal static Activity? CreateTraceActivityForSchedulingSubOrchestration(
        ...
        _ = activity.SetTag(Schema.Task.Type, TraceActivityConstants.Orchestration);
        _ = activity.SetTag(Schema.Task.Name, createdEvent.Name);
        _ = activity.SetTag(Schema.Task.InstanceId, orchestrationInstance?.InstanceId);

        if (!string.IsNullOrEmpty(createdEvent.Version)) {
            _ = activity.SetTag(Schema.Task.Version, createdEvent.Version);
        }
    ...
    internal static Activity? StartTraceActivityForEventRaisedFromWorker(
        ...
        _ = newActivity.AddTag(Schema.Task.Type, TraceActivityConstants.Event);
        _ = newActivity.AddTag(Schema.Task.Name, eventRaisedEvent.Name);
        _ = newActivity.AddTag(Schema.Task.InstanceId, instance?.InstanceId);
        _ = newActivity.AddTag(Schema.Task.ExecutionId, instance?.ExecutionId);

        if (!string.IsNullOrEmpty(targetInstanceId)) {
            _ = newActivity.AddTag(Schema.Task.EventTargetInstanceId, targetInstanceId);
        }
    ...
    internal static void EmitTraceActivityForTimer(
        ...
        if (newActivity is not null) {
            _ = newActivity.AddTag(Schema.Task.Type, TraceActivityConstants.Timer);
            _ = newActivity.AddTag(Schema.Task.Name, orchestrationName);
            _ = newActivity.AddTag(Schema.Task.InstanceId, instance?.InstanceId);
            _ = newActivity.AddTag(Schema.Task.FireAt, timerFiredEvent.FireAt.ToString("o"));
            _ = newActivity.AddTag(Schema.Task.TaskId, timerFiredEvent.TimerId);
    ...
    internal static void SetRuntimeStatusTag(string runtimeStatus) {
        _ = (DistributedTraceActivity.Current?.SetTag(Schema.Task.Status, runtimeStatus));
    }

    internal static void AddErrorDetailsToSpan(Activity? activity, Exception e) {
        _ = (activity?.SetStatus(ActivityStatusCode.Error, e.Message.ToString()));
    }
...
#warning TODO:!
    ...
    ///     Trace a session exception without execution id
    /// </summary>
    public static ExceptionDispatchInfo TraceExceptionSession(
        TraceEventType eventLevel,
        string eventType,
        string sessionId,
