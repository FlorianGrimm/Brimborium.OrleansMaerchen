...
public class ListenerLink : Link
{
    ...
    // link endpoint
    private LinkEndpoint linkEndpoint;

    // send
    private Action<int, Fields, object> onCredit;
    private Action<Message, DeliveryState, bool, object> onDispose;

    // receive
    private bool autoRestore;
    private int restored;
    private MessageDelivery deliveryCurrent;
    private Action<ListenerLink, Message, DeliveryState, object> onMessage;
    ...
    private static void ThrowIfNotNull(object obj, string name)
    {
        if (obj != null)
        {
    ...
    private void DeliverMessage(MessageDelivery messageDelivery)
    {
        var container = ((ListenerConnection)this.Session.Connection).Listener.Container;
    ...
    private void PerformCleanup(Error error)
    {
