...
public static class AmqpEncoder {
    ...
    private static Dictionary<ulong, CreateDescribed> knownDescribedByCode;
        ...
        if (value == null) {
            ...
            if (last < 0 && smallEncoding) {
                ...
                for (int i = 0; i <= last; ++i) {
                    AmqpEncoder.WriteObject(buffer, value[i], smallEncoding);
                }
    ...
    public static void WriteArray(ByteBuffer buffer, Array value) {
        if (value == null) {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                if (i == 0) {
                    AmqpEncoder.WriteObject(buffer, item, false);
                } else {
    ...
    public static void WriteMap(ByteBuffer buffer, Map value, bool smallEncoding) {
        if (value == null) {
            ...
            foreach (var key in value.Keys) {
                AmqpEncoder.WriteObject(buffer, key);
                AmqpEncoder.WriteObject(buffer, value[key]);
            }
    ...
    public static object ReadDescribed(ByteBuffer buffer, byte formatCode) {
        ...
        if (descriptorFormatCode == FormatCode.ULong || descriptorFormatCode == FormatCode.ULong0 || descriptorFormatCode == FormatCode.SmallULong) {
            ...
            descriptor = AmqpEncoder.ReadObject(buffer, descriptorFormatCode);
            create = (CreateDescribed)knownDescribed[descriptor];
        }

        if (create == null) {
            object value = AmqpEncoder.ReadObject(buffer);
    ...
    private static string ReadString(ByteBuffer buffer, byte formatCode, byte code8, byte code32, string type) {
        ...
        string value;
        IStringDecoder localDecoder = AmqpEncoder.stringDecoder; //store in local variable to prevent NullReferenceException if stringDecoder is changed after the null check
