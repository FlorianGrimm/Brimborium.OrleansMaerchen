...
{
    private readonly Dictionary<BufferKey, string> knownStrings = new Dictionary<BufferKey, string>();
    ...
    {
        private readonly ArraySegment<byte> encodedString;
        ...
        public bool Equals(BufferKey other)
        {
            ...
            var thisArray = this.encodedString.Array;
            if (thisArray is null) { return false; }
            var thisOffset = this.encodedString.Offset;

            var otherArray = other.encodedString.Array;
            if (otherArray is null) { return false; }
            var otherOffset = other.encodedString.Offset;

            for (int i = 0; i < this.encodedString.Count; i++)
            {
                if (thisArray[thisOffset + i] != otherArray[otherOffset + i])
                {
                    return false;
                }
            }
