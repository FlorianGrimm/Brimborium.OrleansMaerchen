...
public class Message : IDisposable {
    ...
    public object? Body {
        get {
            if (this.BodySection == null) {
                ...
                if (this.BodySection is DataList dataList) {
    ...
    /// </summary>
    public byte[]? DeliveryTag => this.Delivery?.Tag;
    ...
    private void WriteToBuffer(ByteBuffer buffer) {
        ...
        this.DeliveryAnnotations?.Encode(buffer);
        this.MessageAnnotations?.Encode(buffer);
        this.Properties?.Encode(buffer);
        this.ApplicationProperties?.Encode(buffer);
        this.BodySection?.Encode(buffer);
        this.Footer?.Encode(buffer);
    }
    ...
    private static int GetEstimatedBodySize(RestrictedDescribed body) {
        ...
        if (body is AmqpValue value) {
            ...
            if (value.Value is ByteBuffer f) {
                return f.Length;
            }
        }

        if (body is DataList dataList) {
            int size = 0;
    ...
    private static int GetEstimatedDataSize(Data data) {
        if (data.Buffer != null) {
    ...
    public int GetEstimatedMessageSize() {
        ...
        if (this.BodySection != null) {
            size += GetEstimatedBodySize(this.BodySection) + 8;
        }
