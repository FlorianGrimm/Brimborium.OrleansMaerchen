...
public partial class Connection : AmqpObject {
    ...
    private readonly OnOpened onOpened;
    ...
    private uint remoteMaxFrameSize;
    private ITransport writer;
    private HeartBeat heartBeat;

    private Connection(Address address, ushort channelMax, uint maxFrameSize) {
    ...
    private static ushort GetChannelMax(AmqpSettings amqpSettings, Open open) {
        return open != null ? open.ChannelMax : (ushort)(amqpSettings.MaxSessionsPerConnection - 1);
    }

    private static uint GetMaxFrameSize(AmqpSettings amqpSettings, Open open) {
    ...
    internal void OnException(Exception exception) {
        ...
        Error error = exception is AmqpException amqpException ?
            amqpException.Error :
    ...
    private sealed class HeartBeat {
        ...
        private readonly uint local;  // for enforcing heartbeats
        private uint remote; // for sending heartbeats
        ...
        private static uint GetDueMilliseconds(uint timeout, DateTime now, DateTime last) {
            uint due = uint.MaxValue;
            if (timeout > 0) {
                uint elapsed = (uint)((now.Ticks - last.Ticks) / AmqpEncoder.TicksPerMillisecond);
    ...
    private sealed class Writer : System.Collections.Generic.Queue<ByteBuffer>, ITransport {
