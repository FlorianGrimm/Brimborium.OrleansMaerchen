...
public partial class AmqpObject {
    ...
    internal async Task CloseInternalAsync(int timeout = 60000, Error error = null) {
        ...
        using (CancellationTokenSource cts = new()) {
            Task task = await Task.WhenAny(tcs.Task, Task.Delay(timeout, cts.Token)).ConfigureAwait(false);
            if (task != tcs.Task) {
...
internal class SendTask : TaskCompletionSource<bool> {
...
public static class TaskExtensions {
    internal static async Task<DeliveryState> GetTransactionalStateAsync(SenderLink sender) {
        return await Brimborium.OrleansAmqp.Transactions.ResourceManager.GetTransactionalStateAsync(sender).ConfigureAwait(false);
    }
