...
public partial class ConnectionFactory : ConnectionFactoryBase {
    private readonly Dictionary<string, TransportProvider> transportFactories;
    ...
    private async Task<IAsyncTransport> CreateTransportAsync(Address address, SaslProfile saslProfile, IHandler handler) {
    ...
    private async Task<Connection> CreateAsync(Address address, Open open, OnOpened onOpened, IHandler handler) {
    ...
    public class SslSettings {
        internal const SslProtocols DefaultSslProtocols = SslProtocols.Tls12 | SslProtocols.Tls13;
    ...
    private sealed class ConnectTaskCompletionSource : TaskCompletionSource<Connection> {
        ...
        private void OnOpen(IConnection connection, Open open) {
