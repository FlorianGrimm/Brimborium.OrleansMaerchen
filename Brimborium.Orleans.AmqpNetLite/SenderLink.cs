...
public partial class SenderLink : Link {
    ...
    // outgoing queue
    private readonly SenderSettleMode settleMode;
    private readonly LinkedList outgoingList;
    ...
    private DeliveryState GetTxnState() {
        return Brimborium.OrleansAmqp.Transactions.ResourceManager.GetTransactionalStateAsync(this).Result;
    }
    ...
    public void Cancel(Message message) {
        ...
        if (delivery.BytesTransfered > 0) {
            this.Session.DisposeDelivery(false, delivery, new Released(), true);
        }
    }
    ...
    internal override void OnDeliveryStateChanged(Delivery delivery) {
        ...
        if (delivery.OnOutcome != null) {
            ...
            if (delivery.State != null && delivery.State is Brimborium.OrleansAmqp.Transactions.TransactionalState) {
                outcome = ((Brimborium.OrleansAmqp.Transactions.TransactionalState)delivery.State).Outcome;
            }
