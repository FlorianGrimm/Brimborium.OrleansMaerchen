...
public sealed class TestAmqpBroker : IContainer {
    ...
    public TestAmqpBroker(IList<string> endpoints, string? userInfo, string? certValue, string[]? queues) {
        ...
        this._Queues = new Dictionary<string, TestQueue>();
        if (queues != null) {
    ...
    private sealed class BrokerMessage : Message {
        ...
        private void WriteMessageAnnotations(ref MessageAnnotations? annotations, MessageAnnotations? current, ByteBuffer buffer) {
        ...
        private void Merge(Map source, Map dest) {
    ...
    private sealed class TestQueue {
        private readonly TestAmqpBroker _Broker;
        private readonly string _Address;
        private readonly bool _IsImplicit;
        private readonly HashSet<Connection>? _Connections;
        private readonly LinkedList<BrokerMessage> _Messages;
        private readonly LinkedList<Consumer> _Waiters;
        private readonly Dictionary<int, Publisher> _Publishers;
        private readonly Dictionary<int, Consumer> _Consumers;
        ...
        public TestQueue(TestAmqpBroker broker, string address, bool isImplicit = false) {
            ...
            this._Connections = isImplicit ? new HashSet<Connection>() : null;
            this._Messages = new LinkedList<BrokerMessage>();
            this._Waiters = new LinkedList<Consumer>();
            this._Publishers = new Dictionary<int, Publisher>();
            this._Consumers = new Dictionary<int, Consumer>();
        ...
        private sealed class Consumer {
            ...
            private static void OnDispose(Message message, DeliveryState deliveryState, bool settled, object state) {
                ...
                if (deliveryState is TransactionalState) {
                    Transaction txn = thisPtr._Queue._Broker._TxnManager.GetTransaction(((TransactionalState)deliveryState).TxnId);
                    txn.AddOperation(message, _OnDischarge, thisPtr);
                } else {
